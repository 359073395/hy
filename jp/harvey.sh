...
if [ ! -t 0 ] && [ -r /dev/tty ] && [ "${HARVEY_TTY_REEXEC:-0}" != "1" ]; then
	...
	echo "現在パイプラインで実行されているため、メニューはキーボード入力を読み取ることができません。スクリプトをダウンロードして実行するか、bash <(curl -fsSL https://raw.githubusercontent.com/359073395/hy/main/harvey.sh) を使用してください。"
	exit 1
fi
quanju_canshu() {
if [ "$canshu" = "CN" ]; then
...
Kernel_optimize() {
	...
	while true; do
	  ...
	  local current_mode=$(grep "^# モード:" /etc/sysctl.d/99-harvey-optimize.conf 2>/dev/null | sed 's/# モード: //' | awk -F'|' '{print $1}' | xargs)
...
moltbot_menu() {
	...
	show_menu() {
		...
		echo "======================================="
		echo -e "🦞 HARVEY による OPENCLAW 管理ツール 🦞"
...
while true; do
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}-------------------------"
	  echo -e "${gl_kjlan}117. ${color117}IP品質検査プラットフォーム${gl_huang}★${gl_bai}"
	  echo -e "${gl_kjlan}-------------------------"
	  echo -e "${gl_kjlan}118. ${color118}AimiliVPN プロキシ ゲートウェイ${gl_huang}★${gl_bai}"
	  echo -e "${gl_kjlan}-------------------------"
	  echo -e "${gl_kjlan}119. ${color119}FluxPanel トラフィック転送パネル${gl_huang}★${gl_bai}"
	  echo -e "${gl_kjlan}-------------------------"
	  echo -e "${gl_kjlan}120. ${color120}LittlePrinceAgent クラウド アシスタント${gl_huang}★${gl_bai}"
	  echo -e "${gl_kjlan}-------------------------"
	  echo -e "${gl_kjlan}121. ${color121}シャドウチェーンワークショップのビデオ分析${gl_huang}★${gl_bai}"
	  echo -e "${gl_kjlan}-------------------------"
	  echo -e "${gl_kjlan}122. ${color122}トラフィックIP核爆発エンジン${gl_huang}★${gl_bai}"
	  echo -e "${gl_kjlan}-------------------------"
		...
		while true; do
			...
			case $sub_choice in
				...
				3)
					...
					sed -i "/\b${app_id}\b/d" /home/docker/appno.txt
					echo "アンインストールが完了しました"
					send_stats "AimiliVPNをアンインストールする"
					;;
				5)
...
check_port_conflict 6365 6366 && { open_port 6365; open_port 6366; } || { echo "インストールがキャンセルされました"; break; }
	  ...
	  121|yt|video-parser|yinglian)
		local app_id="121"
		send_stats "シャドウチェーンワークショップのビデオ分析"
		while true; do
			clear
			local check_status="${gl_hui}インストールされていません${gl_bai}"
			...
			echo -e "シャドウチェーンワークショップのビデオ分析$check_status"
			echo "公式サイト：https://github.com/359073395/yt"
			echo "パブリックビデオ分析およびサーバー一時ダウンロードツール、デフォルトポート:$yt_port"
			echo "デフォルトの管理者: admin / lhw111111"
			echo ""
			if [ -d /opt/video-parser/project/video-parser ]; then
				ip_address
				if [ -n "$ipv4_address" ]; then
					echo "Web管理パネル: http://$ipv4_address:$yt_port"
					echo "ヘルスチェック: http://$ipv4_address:$yt_port/api/health"
				fi
				for file in /home/web/conf.d/*; do
			...
			case $sub_choice in
				1)
					check_disk_space 2
					install curl
					check_port_conflict 9890 && { open_port 9890; } || { echo "インストールがキャンセルされました"; break; }
					bash <(curl -fsSL https://raw.githubusercontent.com/359073395/yt/main/project/video-parser/deploy/install.sh) --port 9890
					add_app_id
					send_stats "シャドウチェーンワークショップをインストールする"
					;;
				2)
					install curl
					bash <(curl -fsSL https://raw.githubusercontent.com/359073395/yt/main/project/video-parser/deploy/install.sh) --port 9890
					add_app_id
					send_stats "シャドウチェーンワークショップを更新"
					;;
				3)
					if [ -f /opt/video-parser/project/video-parser/.env ]; then
						yt_port=$(grep "^PUBLIC_PORT=" /opt/video-parser/project/video-parser/.env 2>/dev/null | tail -n 1 | cut -d= -f2-)
					...
					sed -i "/\b${app_id}\b/d" /home/docker/appno.txt
					echo "アンインストールが完了しました"
					send_stats "シャドウチェーンワークショップをアンインストールする"
					;;
				5)
					send_stats "Shadow Chain Workshop のドメイン名アクセス"
					add_yuming
	  ...
	  122|ip-commerce|ipcg|liuliangip)
		...
		ipcg_deploy() {
			...
			if [ -n "$ipcg_latest_sha" ]; then
				...
				echo "解析できません${ipcg_repo}最新の提出物は引き続き使用されます${ipcg_ref}"
			fi
			echo "導入の準備をする${ipcg_repo}@${ipcg_ref}"
			curl -fsSL \
				...
				-o "$ipcg_installer" || { echo "スクリプトのダウンロードとインストールに失敗しました。確認してください。${ipcg_repo}パブリックまたは Web アクセス可能"; return 1; }
		...
		while true; do
			...
			echo -e "トラフィックIP核爆発エンジン$check_status"
			echo "公式サイト：https://github.com/359073395/ip-commerce-generator"
			echo "ナレッジベースに基づいた IP の位置付け、トピックの選択、スクリプトと配信計画のジェネレーター、デフォルトのポート:$ipcg_port"
			echo ""
			if [ -d "$ipcg_dir" ]; then
				ip_address
				if [ -n "$ipv4_address" ]; then
					echo "Web管理パネル: http://$ipv4_address:$ipcg_port"
					echo "ヘルスチェック: http://$ipv4_address:$ipcg_port/api/health"
				fi
				for file in /home/web/conf.d/*; do
			...
			case $sub_choice in
				1)
					check_disk_space 2
					install curl git
					check_port_conflict 8790 && { open_port 8790; } || { echo "インストールがキャンセルされました"; break; }
					if ipcg_deploy 1; then
						add_app_id
						send_stats "トラフィックIP核爆発エンジンを設置"
					else
						echo "トラフィック IP 核爆発エンジンのインストールに失敗しました。ネットワークまたは上記のエラー ログを確認してください。"
					fi
					;;
				2)
					install curl git
					if ipcg_deploy 0; then
						add_app_id
						send_stats "トラフィック IP 核爆発エンジンを更新する"
					else
						echo "トラフィック IP 核爆発エンジンの更新に失敗しました。ネットワークまたは上記のエラー ログを確認してください。"
					fi
					;;
				3)
					if [ -f "$ipcg_dir/.env" ]; then
					...
					sed -i "/\b${app_id}\b/d" /home/docker/appno.txt
					echo "アンインストールが完了しました"
					send_stats "トラフィック IP 核爆発エンジンをアンインストールする"
					;;
				5)
					send_stats "トラフィック IP 核爆発エンジン ドメイン名アクセス"
					add_yuming
...
linux_Settings() {
	while true; do
	  ...
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}66.  ${gl_bai}ワンストップのシステムチューニング${gl_huang}★${gl_bai}"
	  echo -e "${gl_kjlan}99.  ${gl_bai}サーバーを再起動します${gl_kjlan}100. ${gl_bai}プライバシーとセキュリティ"
	  echo -e "${gl_kjlan}101. ${gl_bai}k コマンドの高度な使用法${gl_huang}★${gl_bai}                    ${gl_kjlan}102. ${gl_bai}Harvey スクリプトをアンインストールする"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}0.   ${gl_bai}メインメニューに戻る"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  read -e -p "選択肢を入力してください:" sub_choice
...
echo -e "${gl_bai}ウェブサイト: https://www.gname.com/register?tt=86836&ttcode=HARVEY868​​36&ttbj=sh${gl_bai}"
echo "------------------------"
echo ""
echo -e "ハーベイ周辺"
echo "------------------------"
echo -e "${gl_kjlan}ステーションB:${gl_bai}https://b23.tv/2mqnQyh              ${gl_kjlan}オイルパイプ：${gl_bai}https://www.youtube.com/@harvey${gl_bai}"
...
echo "------------------------"
echo -e "${gl_kjlan}スクリプト公式サイト：${gl_bai}https://sh.hyjiexi.eu.org            ${gl_kjlan}GitHub アドレス:${gl_bai}${gh_https_url}github.com/359073395/hy${gl_bai}"
echo "------------------------"
echo ""
}
games_server_tools() {
...
while true; do
	...
	echo "変更ログ"
	echo "------------------------"
	echo "すべてのログ:${gh_proxy}raw.githubusercontent.com/359073395/hy/main/harvey_sh_log.txt"
	echo "------------------------"
...
clear
echo -e "${gl_kjlan}"
echo -e "Harvey スクリプト ツールボックス v$sh_v"
...
echo "アプリケーションマーケット管理Hアプリ"
echo "申請番号の迅速な管理 H アプリ 26 | Hアプリ1パネル | H アプリ npm"
