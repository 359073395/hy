...
UserLicenseAgreement() {
	...
	echo "ユーザー使用許諾契約: https://blog.harvey.pro/user-license-agreement/"
	echo -e "----------------------"
...
sshkey_panel() {
  ...
  while true; do
	  ...
	  if [[ "$REAL_STATUS" =~ "yes" ]]; then
  	  ...
  	  echo "高度なゲームプレイ: https://blog.harvey.pro/ssh-key"
  	  echo "------------------------------------------------"
...
while true; do
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}サードパーティ製アプリケーションのリスト"
  	  echo -e "${gl_kjlan}あなたのアプリをここに表示したいですか?開発者ガイドを確認してください。${gl_huang}https://dev.harvey.sh/${gl_bai}"
...
EOF
		  ...
		  21)
			  ...
			  while true; do
				  ...
				  case $host_dns in
					  1)
						  read -e -p "新しい解析レコード形式を入力してください: 110.25.5.33 harvey.pro:" addhost
...
echo -e "${gl_bai}ウェブサイト: https://harvey.pro/topvps/${gl_bai}"
echo "------------------------"
echo ""
echo -e "ドメイン名の割引"
echo "------------------------"
...
echo -e "${gl_kjlan}公式ウェブサイト:${gl_bai}https://harvey.pro/              ${gl_kjlan}ナビゲーション:${gl_bai}https://dh.harvey.pro/${gl_bai}"
echo -e "${gl_kjlan}ブログ:${gl_bai}https://blog.harvey.pro/         ${gl_kjlan}ソフトウェアセンター:${gl_bai}https://app.harvey.pro/${gl_bai}"
echo "------------------------"
echo -e "${gl_kjlan}スクリプト公式サイト：${gl_bai}https://harvey.sh            ${gl_kjlan}GitHub アドレス:${gl_bai}${gh_https_url}github.com/359073395/hy${gl_bai}"
echo "------------------------"
echo ""
}
games_server_tools() {
...
while true; do
	...
	echo "変更ログ"
	echo "------------------------"
	echo "すべてのログ:${gh_proxy}raw.githubusercontent.com/harvey/sh/main/harvey_sh_log.txt"
	echo "------------------------"
