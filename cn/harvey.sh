...
if [ ! -t 0 ] && [ -r /dev/tty ] && [ "${HARVEY_TTY_REEXEC:-0}" != "1" ]; then
	tmp_script="${TMPDIR:-/tmp}/harvey-cn-tty-$$.sh"
	if command -v curl >/dev/null 2>&1; then
		if curl -fsSL "https://raw.githubusercontent.com/359073395/hy/main/cn/harvey.sh?ts=$(date +%s)" -o "$tmp_script"; then
			chmod +x "$tmp_script" && HARVEY_TTY_REEXEC=1 exec bash "$tmp_script" "$@" </dev/tty
		fi
	fi
	if command -v wget >/dev/null 2>&1; then
		if wget -qO "$tmp_script" "https://raw.githubusercontent.com/359073395/hy/main/cn/harvey.sh?ts=$(date +%s)"; then
			chmod +x "$tmp_script" && HARVEY_TTY_REEXEC=1 exec bash "$tmp_script" "$@" </dev/tty
		fi
	fi
	echo "当前以管道方式运行，菜单无法读取键盘输入。请下载脚本后运行，或使用: bash <(curl -fsSL https://raw.githubusercontent.com/359073395/hy/main/cn/harvey.sh)"
	exit 1
fi
quanju_canshu() {
if [ "$canshu" = "CN" ]; then
...
root_use() {
clear
[ "$EUID" -ne 0 ] && echo -e "${gl_huang}提示: ${gl_bai}该功能需要root用户才能运行！" && break_end && harvey
}
dd_xitong() {
		send_stats "重装系统"
		dd_xitong_MollyLau() {
...
while true; do
	...
	case $sub_choice in
	  ...
	  82|gpt-load)
		...
		docker_rum() {
			read -e -p "设置${docker_name}的登录密钥（sk-开头字母和数字组合）如: sk-159harveyyyds163: " app_passwd
...
linux_Settings() {
	while true; do
	  ...
	  echo -e "${gl_kjlan}101. ${gl_bai}k命令高级用法 ${gl_huang}★${gl_bai}                    ${gl_kjlan}102. ${gl_bai}卸载Harvey脚本"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}0.   ${gl_bai}返回主菜单"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  read -e -p "请输入你的选择: " sub_choice
...
EOF
		  ...
		  102)
			  clear
			  send_stats "卸载Harvey脚本"
			  echo "卸载Harvey脚本"
			  echo "------------------------------------------------"
			  echo "将彻底卸载harvey脚本，不影响你其他功能"
			  read -e -p "确定继续吗？(Y/N): " choice
			  case "$choice" in
				[Yy])
				  clear
...
while true; do
	  ...
	  echo -e "${gl_kjlan}11. ${gl_bai}安装Harvey脚本         ${gl_kjlan}12. ${gl_bai}更新系统              ${gl_kjlan}13. ${gl_bai}清理系统"
...
echo "------------------------"
echo -e "${gl_kjlan}脚本官网: ${gl_bai}https://sh.hyjiexi.eu.org            ${gl_kjlan}GitHub地址: ${gl_bai}${gh_https_url}github.com/359073395/hy${gl_bai}"
echo "------------------------"
echo ""
}
games_server_tools() {
...
while true; do
	...
	echo "更新日志"
	echo "------------------------"
	echo "全部日志: ${gh_proxy}raw.githubusercontent.com/359073395/hy/main/harvey_sh_log.txt"
	echo "------------------------"
...
clear
echo -e "${gl_kjlan}"
echo -e "Harvey脚本工具箱 v$sh_v"
echo -e "命令行输入${gl_huang}H${gl_kjlan}可快速启动脚本${gl_bai}"
echo -e "${gl_kjlan}------------------------${gl_bai}"
