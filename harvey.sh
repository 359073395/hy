...
UserLicenseAgreement() {
	...
	echo "用户许可协议: https://blog.harvey.pro/user-license-agreement/"
	echo -e "----------------------"
...
sshkey_panel() {
  ...
  while true; do
	  ...
	  if [[ "$REAL_STATUS" =~ "yes" ]]; then
  	  ...
  	  echo "进阶玩法: https://blog.harvey.pro/ssh-key"
  	  echo "------------------------------------------------"
...
EOF
		  ...
		  21)
			  ...
			  while true; do
				  ...
				  case $host_dns in
					  1)
						  read -e -p "请输入新的解析记录 格式: 110.25.5.33 harvey.pro : " addhost
...
echo -e "${gl_bai}网址: https://harvey.pro/topvps/${gl_bai}"
echo "------------------------"
echo ""
echo -e "域名优惠"
echo "------------------------"
