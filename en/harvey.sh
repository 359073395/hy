...
if [ ! -t 0 ] && [ -r /dev/tty ] && [ "${HARVEY_TTY_REEXEC:-0}" != "1" ]; then
	...
	echo "Currently running in a pipeline, the menu cannot read keyboard input. Please download the script and run it, or use: bash <(curl -fsSL https://raw.githubusercontent.com/359073395/hy/main/harvey.sh)"
	exit 1
fi
quanju_canshu() {
if [ "$canshu" = "CN" ]; then
...
Kernel_optimize() {
	...
	while true; do
	  ...
	  local current_mode=$(grep "^# Mode:" /etc/sysctl.d/99-harvey-optimize.conf 2>/dev/null | sed 's/# mode: //' | awk -F'|' '{print $1}' | xargs)
...
while true; do
	...
	case $sub_choice in
	  ...
	  11|zentao)
		...
		docker_rum() {
			read -e -p "Set Zentao admin password: " zentao_passwd
		...
		local docker_passwd="echo \"Admin password was set during deployment\""
		local app_size="2"
		docker_app
		  ;;
	  12|qinglong)
	  ...
	  122|ip-commerce|ipcg|liuliangip)
		...
		ipcg_deploy() {
			...
			if [ -n "$ipcg_latest_sha" ]; then
				...
				echo "Unable to parse${ipcg_repo}Latest submission, will continue to be used${ipcg_ref}"
			fi
			echo "Prepare to deploy${ipcg_repo}@${ipcg_ref}"
			curl -fsSL \
...
linux_Settings() {
	while true; do
	  ...
	  echo -e "${gl_kjlan}101. ${gl_bai}Advanced usage of k command${gl_huang}★${gl_bai}                    ${gl_kjlan}102. ${gl_bai}Uninstall Harvey script"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}0.   ${gl_bai}Return to main menu"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  read -e -p "Please enter your choice:" sub_choice
...
EOF
		  ...
		  102)
			  clear
			  send_stats "Uninstall Harvey script"
			  echo "Uninstall Harvey script"
			  echo "------------------------------------------------"
...
echo -e "${gl_bai}Website: https://www.gname.com/register?tt=86836&ttcode=HARVEY86836&ttbj=sh${gl_bai}"
echo "------------------------"
echo ""
