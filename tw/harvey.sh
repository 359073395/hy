...
linux_Settings() {
	while true; do
	  ...
	  echo -e "${gl_kjlan}101. ${gl_bai}k指令進階用法${gl_huang}★${gl_bai}                    ${gl_kjlan}102. ${gl_bai}卸载Harvey脚本"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}0.   ${gl_bai}返回主選單"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  read -e -p "請輸入你的選擇:" sub_choice
...
echo "------------------------"
echo -e "${gl_kjlan}腳本官網:${gl_bai}https://harvey.sh            ${gl_kjlan}GitHub地址:${gl_bai}${gh_https_url}github.com/359073395/hy${gl_bai}"
echo "------------------------"
echo ""
}
games_server_tools() {
...
while true; do
	...
	echo "更新日誌"
	echo "------------------------"
	echo "全部日誌:${gh_proxy}raw.githubusercontent.com/harvey/sh/main/harvey_sh_log.txt"
	echo "------------------------"
